...
ACTION_MAP = {"stand": 0, "squat": 1, "one-foot-stand": 2,
              "open-arms": 3, "arms-skimbo": 4}
...
def annotate_action():
    ...
    '''
    with os.scandir(pic_with_keypoints_root) as entries:
        task_path = [
            os.path.join(pic_with_keypoints_root, video_name, "classification")
            for video_name in entries if video_name.is_dir()
    ]
    database_path = os.path.join(pic_with_keypoints_root, "database")
    num_labels = 0
    for action_root_path in task_path:
        img_path = os.path.join(database_path, "images")
        label_path = os.path.join(database_path, "labels")
        check_dirs([database_path, img_path, label_path])
        for action in ALL_ACTIONS:
            action_dir = os.path.join(action_root_path, action)
            if not os.path.exists(action_dir):
                continue
            with tqdm (total=len(os.listdir(action_dir)), 
                       desc=f"Annotating {os.path.basename(os.path.dirname(action_root_path))}-{action}", 
                       leave=False) as pbar_img:
                for file in os.listdir(action_dir):
                    if file.endswith(".yaml"):
                        action_labeled_file = os.path.join(action_dir, file)
                        with open(action_labeled_file, 'r') as f:
                        ...
                        points = []
                        for kp in data["keypoints"][0]["points"]:
                            point_data = {
                                "x": kp["x"],
                                "y": kp["y"],
                                "vx": kp["vx"],
                        ...
                        label_info["kps"] = points
                        num_labels += 1
                        label_file = os.path.join(label_path, f"{num_labels}.yaml")
                        with open(label_file, 'w') as f:
                            yaml.dump(label_info, f, default_flow_style=False)
                        img_src_path = os.path.join(os.path.dirname(action_labeled_file),
                                                    os.path.basename(action_labeled_file).split(".")[0] + ".jpg")
                        img_dst_path = os.path.join(img_path,
                                                    f"{num_labels}.jpg")
